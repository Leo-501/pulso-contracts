export class Projecao {
     ...
     */
    constructor(db, aoAplicarQuadro) {
        this.db = db;
        this.aoAplicarQuadro = aoAplicarQuadro;
    }
    /**
    ...
    async garantir(contexto) {
        ...
        await this.projetando(contexto, async (tx) => {
            await tx.query(`INSERT INTO empresas(id,apelido,nome) VALUES($1,$2,$3)
         ON CONFLICT(id) DO UPDATE SET nome=EXCLUDED.nome`, [contexto.empresa_id, apelidoDe(contexto.empresa_id), contexto.empresa_nome]);
            ...
            await tx.query(`INSERT INTO unidades(id,empresa_id,nome,cidade,fuso) VALUES($1,$2,$3,'',$4)
         ON CONFLICT(id) DO UPDATE SET nome=EXCLUDED.nome,fuso=EXCLUDED.fuso`, [contexto.unidade_id, contexto.empresa_id, contexto.unidade_nome, contexto.fuso]);
    ...
    async aplicar(sessao, quadro) {
        await this.projetando(sessao, async (tx) => {
            await tx.query(`INSERT INTO empresas(id,apelido,nome) VALUES($1,$2,$3)
         ON CONFLICT(id) DO UPDATE SET nome=EXCLUDED.nome`, [sessao.empresa_id, apelidoDe(sessao.empresa_id), sessao.empresa_nome]);
            for (const unidade of quadro.unidades)
                await tx.query(`INSERT INTO unidades(id,empresa_id,nome,cidade,fuso) VALUES($1,$2,$3,$4,$5)
           ON CONFLICT(id) DO UPDATE SET nome=EXCLUDED.nome,cidade=EXCLUDED.cidade,fuso=EXCLUDED.fuso`, [unidade.id, sessao.empresa_id, unidade.nome, unidade.cidade, unidade.fuso]);
    ...
    async pessoa(tx, p) {
        ...
        await tx.query('INSERT INTO pessoas(id,email,nome,ativo) VALUES($1,$2,$3,$4) ON CONFLICT(id) DO NOTHING', [p.id, p.email.toLowerCase(), p.nome, p.ativo]);
        await tx.query(`INSERT INTO vinculos(empresa_id,pessoa_id,papel,ativo) VALUES($1,$2,$3,$4)
       ON CONFLICT(empresa_id,pessoa_id) DO UPDATE SET papel=EXCLUDED.papel,ativo=EXCLUDED.ativo`, [p.empresa, p.id, p.papel, p.vinculo_ativo]);
        ...
        await tx.query('DELETE FROM vinculo_unidades WHERE empresa_id=$1 AND pessoa_id=$2 AND NOT (unidade_id = ANY($3::uuid[]))', [p.empresa, p.id, p.unidade_ids]);
        for (const unidade of p.unidade_ids)
            await tx.query(`INSERT INTO vinculo_unidades(empresa_id,pessoa_id,unidade_id) VALUES($1,$2,$3)
         ON CONFLICT DO NOTHING`, [p.empresa, p.id, unidade]);
    }
}
/**
 ...
 */
const apelidoDe = (empresaId) => 'id:' + empresaId;
//# sourceMappingURL=projecao.js.map