import { esquemaLogin, esquemaPessoa, esquemaSenha, esquemaUnidade, esquemaVinculo, } from '../index.js';
...
export function criarIdentidadeDuplo(spec) {
    ...
    for (const e of spec.empresas) {
        const empresa = { id: uuid(), apelido: e.apelido, nome: e.nome };
    ...
    for (const p of spec.pessoas) {
        ...
        for (const v of p.vinculos) {
            ...
            if (!empresa)
                throw new Error(`empresa desconhecida no duplo: ${v.empresa}`);
            const nomes = v.unidades ?? spec.empresas.find((e) => e.apelido === v.empresa).unidades.map((u) => u.nome);
    ...
    const porId = (id) => [...pessoas.values()].find((p) => p.id === id);
    const unidadePorId = (id) => [...unidades.values()].find((u) => u.id === id);
    const empresaPorId = (id) => [...empresas.values()].find((e) => e.id === id);
    /** Unidade de entrada: a primeira em ordem alfabética, nunca a de menor uuid. */
    const entrada = (vinculo) => [...vinculo.unidades]
        .map((id) => unidadePorId(id))
        .sort((a, b) => a.nome.localeCompare(b.nome) || a.id.localeCompare(b.id))[0];
    const contextoDe = (sessao) => {
    ...
    const viva = (sessao, tipo) => {
        if (!sessao || sessao.tipo !== tipo || sessao.expira <= Date.now())
            return undefined;
        const pessoa = porId(sessao.pessoa);
        const vinculo = pessoa?.vinculos.get(sessao.empresa);
        if (!pessoa?.ativo || !vinculo?.ativo)
            return undefined;
        if (!vinculo.unidades.has(sessao.unidade))
            return undefined;
        return sessao;
    };
    const emitir = (pessoa, empresa, unidade, tipo) => {
        const sessao = {
            token: [...Array(8)]
                .map(() => uuid().replace(/-/g, ''))
                .join('')
    ...
    function autenticar(corpo, aplicativo) {
        ...
        if (usadas > 10)
            throw new ErroHttp(403, 'Muitas tentativas. Aguarde 15 minutos.');
        ...
        if (aplicativo && !['tecnico', 'solicitante'].includes(vinculo.papel))
            throw new ErroHttp(403, 'Este aplicativo atende técnicos e solicitantes. Use o painel para os demais perfis.');
        tentativas.delete(chave);
        return emitir(pessoa, empresa.id, entrada(vinculo).id, aplicativo ? 'aplicativo' : 'painel');
    }
    function sessaoDe(cabecalhos) {
    ...
    function unidadesPermitidas(sessao, ids) {
    ...
    function rotear(metodo, caminho, corpo, cabecalhos) {
        ...
        if (metodo === 'POST' && caminho === '/api/entrar')
            return autenticar(corpo, false);
        if (metodo === 'POST' && caminho === '/api/entrar/aplicativo')
            return autenticar(corpo, true);
        ...
        if (metodo === 'POST' && alvo) {
            ...
            const vinculo = pessoa?.vinculos.get(sessao.empresa);
            if (!pessoa || !vinculo)
                throw new ErroHttp(404, 'Vínculo não encontrado nesta empresa.');
            if (alvo[2]) {
                const senha = temporaria();
                pessoa.senha = senha;
            ...
            if (!Object.keys(dados).length)
                throw new ErroHttp(400, 'Informe o que deve mudar.');
            if (id === sessao.pessoa &&
    ...
    const fetchImpl = (async (entrada, init = {}) => {
        ...
        if (cabecalhos.get('x-pulso-cliente') !== cliente.id ||
...
const json = (status, corpo) => new Response(JSON.stringify(corpo ?? null), {
    status,
