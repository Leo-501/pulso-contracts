...
export const rotulosPrioridade = {
    critica: 'Crítica',
    alta: 'Alta',
    media: 'Média',
    baixa: 'Baixa',
};
export const rotulosPapel = {
...
const obrigatorio = (max = 200) => z.string().trim().min(2, 'Informe pelo menos 2 caracteres.').max(max);
...
export function transicoesPermitidas(situacao, papel) {
    if (papel === 'administrador' || papel === 'gestor')
        return transicoes[situacao];
    if (papel === 'tecnico')
        return transicoes[situacao].filter((s) => ['em_execucao', 'pausada', 'validacao'].includes(s) && situacao !== 'validacao');
    return [];
}
...
export function dataOcorrencia(base, frequencia, intervalo, indice) {
    if (frequencia === 'semanal')
        return somarDias(base, 7 * intervalo * indice);
